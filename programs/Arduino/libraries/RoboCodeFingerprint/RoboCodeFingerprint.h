...
class RoboCodeFingerprint {
 public:
#if defined(__AVR__) || defined(ESP8266) || defined(FREEDOM_E300_HIFIVE1)
  RoboCodeFingerprint(uint8_t rx, uint8_t tx, uint32_t password = 0x0);
#endif
  RoboCodeFingerprint(HardwareSerial *hs, uint32_t password = 0x0);
  ...
  int getFingerID();
  int writeNewFinger(int id);
  int deleteFinger(int id);
  bool searchFinger(int id);
