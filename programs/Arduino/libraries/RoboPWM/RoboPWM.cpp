...
void defaultPWM(byte pin) { // тут настроен максимально дефолтные настройки таймера, делитель 64
	switch (pin) {
...
void setPWM(byte pin , unsigned int duty) { // имеем баг с таймерами в режиме fastPWM с прямым шим и исправляем его супер образом
...
void modePWM(byte pin, uint8_t mode) {		// 0 - FastPWM, 1 - Phase-correct PWM
	if (pin == 5 || pin == 6) {
